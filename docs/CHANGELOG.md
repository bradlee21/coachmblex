...
## 2026-02-27
- SLICE-MIDTERM-DISTRACTOR-1: Rebuilt distractors for `src/content/packs/physiology-midterm-v1.json` using deterministic topic buckets (cells/organelles, chemistry, tissues, integumentary, skeletal, joints, muscular, nervous/brain, special senses, anatomy basics) so wrong options are chapter-consistent and plausible. Prompts and correct answers were preserved; strict import passed clean (`Inserted: 109`, `Sanity flagged: 0`, `Linter warnings: 0`).
- SLICE-MIDTERM-D1-FIX-1: Updated `src/content/packs/physiology-midterm-v1.json` for importer compatibility without changing prompts or correct answers: set all question `domain_code` values to `D1`, enforced `question_type: "mcq"`, normalized `answer` to exactly match `choices[correct_choice]`, and updated pack display metadata (`title`, `meta.visibility`, `meta.domain_code`, `meta.domain_label`). Strict import then passed with `Inserted: 109`, `Sanity flagged: 0`, and `Linter warnings: 0`.
- SLICE-D2-V2-1: Added `src/content/packs/mblex-d2-kinesiology-v2.json` as an applied, massage-relevant D2 pack (`50` MCQ; `12 easy / 26 medium / 12 hard`) with `meta.replaces: ["mblex-d2-kinesiology-v1"]`. Archived `mblex-d2-kinesiology-v1` by setting `meta.visibility: "archived"`. Strict import for v2 passed clean (`Linter warnings: 0`, `Sanity flagged: 0`, `Inserted: 50`).
- SLICE-D2-V1-1: Added new pack `src/content/packs/mblex-d2-kinesiology-v1.json` (`source: "codex-generated"`) with 50 D2 MCQs in house format (`12 easy / 26 medium / 12 hard`) covering applied planes/axes, scapulohumeral mechanics, gait deviations, posture patterns, ROM test logic, stretch-vs-strengthen decisions, and limited nerve-entrapment differentiation. Strict import passed clean (`Linter warnings: 0`, `Sanity flagged: 0`, `Inserted: 50`).
- SLICE-NEXT16-PROXY-1: Migrated deprecated Next.js `middleware` file convention to the new `proxy` convention with no behavior changes. Renamed `middleware.js` to `proxy.js`, renamed the exported handler to `proxy`, and preserved identical redirect matcher logic for removed legacy routes to `/today`.
- SLICE-TEST-DOMAIN-INFER-1: Added shared pack-domain metadata utility `src/lib/packDomainMeta.mjs` with authoritative `D1..D7 -> domain_label` mapping and deterministic domain code inference (`meta.domain_code` -> first question `domain_code` -> `pack_id` `mblex-dN-`). `/test` pack loading now auto-resolves `domainCode` and `domainLabel` with fallback to title, and pack search now also matches `domainCode`. Importer now warns when `meta.domain_label` is missing but inferable from domain code.
- SLICE-TEST-DOMAIN-LABEL-1: Updated `/test` pack naming UX to use `meta.domain_label` as the primary label, with existing title (or pack id) as secondary subtext. Added `meta.domain_label` for active packs `mblex-d1-anatomy-physiology-v3` (`Anatomy & Physiology`) and `mblex-d3-advanced-v2` (`Pathology / Contraindications / Special Populations`). Pack filtering now matches domain label, title, and pack id, and missing `meta.domain_label` falls back to title.
